...
dataset.t <- dataset
dataset1.t <- dataset1
#dataset.t$avg_homevalue1000_int <- log(dataset.t$avg_homevalue1000_int) #Average Home Value
#dataset.t$median_income1000_int <- log(dataset.t$median_income1000_int) # Median Family Income
#dataset.t$avg_income1000_int <- log(dataset.t$avg_income1000_int) #Average Family Income
dataset_tv <- dataset.t[dataset$part!="test",]
dataset1_tv <- dataset1.t[dataset1$part!="test",]
dim(dataset_tv) #6002 rows and 24 varaibles
#dataset_train <- dataset_tv[dataset_tv$part=="train",]
#dim(dataset_train) #3984 rows and 24 columns
#dataset_validation <- dataset_tv[dataset_tv$part=="valid",]
#dim(dataset_validation) #2018 rows and 24 columns
#dataset_test <- dataset.t[dataset$part=="test",]
#dim(dataset_test) #2007 rows and 24 varaibles
...
#                  0 737  7
#                 1  282  992
# check n.mail.valid = 282+992 = 1274
# check profit = 14.5*981-2*1274 = 11836
(737+992)/2018#accuracy: .8567
...
#for probabilities we can do AUC
auc <- roc(ifelse(dataset_validation.c[,outcomeName]=="yes",1,0), predictions[[2]]) #use the probability of yes in column 2
print(auc$auc) #Area under the curve: 0.965
...
#                  0 761  6
#                  1 258  993
# check n.mail.valid = 258+993 = 1251
# check profit = 14.5*981-2*1251 = 11896.6
(761+993)/2018#accuracy: .8305
...
# based on this model we'll mail to the 308 highest posterior probabilities
...
###Standardization
# calculate the pre-process parameters from the dataset - preProcess() can pass categorical variables that's why we put the whole dataset
preprocessParams.y <- preProcess(dataset.y[,1:21], method=c("center", "scale"))
# summarize transform parameters
...
# before transformation
sapply(dataset.y[,1:21], sd)
# after transformation
sapply(transformed.y[,1:21], sd)

# create  density plots before transformation
ggplot(melt(dataset.y[,1:21]),aes(x = value)) + 
  facet_wrap(~variable,scales = "free_x")+
  geom_density()
# create  density plots after transformation
ggplot(melt(transformed.y[,1:21]),aes(x = value)) + 
  facet_wrap(~variable,scales = "free_x")+
  geom_density()
...
sapply(dataset.y[,1:21], sd)
# after transformation
sapply(transformed.box.y[,1:21], sd)
...
print(fit.svm.y.box) #c= 1, sigma=0.05600562
# 10-fold cross-validation with 3 repeats
trainControl <- trainControl(method="repeatedcv", number=10, repeats=3)
metric <- "RMSE"
...
grid <- expand.grid(.sigma=c(0.025, 0.05, 0.1, 0.15), .C=seq(1, 10, by=1))
# add tuneGrid for SVM
fit.svm.y.box.tuned <- train(donation_amount_prediction_int~., data=dataset_train.y.box_clean, method="svmRadial", metric=metric,
                 tuneGrid=grid,trControl=trainControl, na.action=na.omit)
print(fit.svm.y.box.tuned)
plot(fit.svm.y.box.tuned)

######glmnet Tuning - Using tuneLength
#what are the default parameteres
print(fit.glmnet.y.box)
trainControl <- trainControl(method="repeatedcv", number=10, repeats=3)
metric <- "RMSE"
set.seed(7)
fit.glmnet.y.box.tuned <- train(donation_amount_prediction_int~., data=dataset_train.y.box_clean, method="glmnet", metric=metric,
                    tuneLength=10,trControl=trainControl, na.action=na.omit)
print(fit.glmnet.y.box.tuned) #did not improve our model
plot(fit.glmnet.y.box.tuned)
...
# Random Forest
set.seed(7)
fit.rf.y.box <- train(donation_amount_prediction_int~., data=dataset_train.y.box_clean, method="rf", metric=metric, trControl=trainControl)
# Stochastic Gradient Boosting
set.seed(7)
fit.gbm.y.box <- train(donation_amount_prediction_int~., data=dataset_train.y.box_clean, method="gbm", metric=metric, trControl=trainControl, verbose=FALSE)
# Cubist
set.seed(7)
fit.cubist.y.box <- train(donation_amount_prediction_int~., data=dataset_train.y.box_clean, method="cubist", metric=metric, trControl=trainControl)
# Compare algorithms
ensembleResults.y <- resamples(list(RF=fit.rf.y.box, GBM=fit.gbm.y.box, CUBIST=fit.cubist.y.box))
...
fit.cubist.y.box.tunned <- train(donation_amount_prediction_int~., data=dataset_train.y.box_clean, method="cubist", metric=metric,
                                 tuneGrid=grid, trControl=trainControl)
print(fit.cubist.y.box.tunned)
plot(fit.cubist.y.box.tunned)
...
##############Gradient Boosting Machines (GBM), RMSE = 0.1134
pred.valid.ls1 <- predict(fit.gbm.y.box, newdata = dataset_validation.y.box) # validation predictions
mean((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2) # mean prediction error
#0.01474
rmse <- RMSE(pred.valid.ls1, dataset_validation.y.box$donation_amount_prediction_int)
rmse
# 0.1214
sd((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2)/sqrt(n.valid.y) # std error
# 0.00082

################Cubist boosting (CUBIST), RMSE = 0.1137
pred.valid.ls1 <- predict(fit.cubist.y.box, newdata = dataset_validation.y.box) # validation predictions
mean((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2) # mean prediction error
#0.01459
sd((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2)/sqrt(n.valid.y) # std error
# 0.00082
...
#################Random Forest, bagging (RF), RMSE = 0.1162
pred.valid.ls1 <- predict(fit.rf.y.box, newdata = dataset_validation.y.box) # validation predictions
mean((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2) # mean prediction error
#0.01523
sd((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2)/sqrt(n.valid.y) # std error
# 0.0008
...
rmse
# 0.1234
##################Penalized Linear Regression (GLMNET), RMSE = 0.1163 
pred.valid.ls1 <- predict(fit.glmnet.y.box, newdata = dataset_validation.y.box) # validation predictions
mean((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2) # mean prediction error
#0.01542
sd((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2)/sqrt(n.valid.y) # std error
# 0.0008
...
rmse
# 0.1242
###################Linear Regression (LM), RMSE= 0.1163
pred.valid.ls1 <- predict(fit.lm.y.box, newdata = dataset_validation.y.box) # validation predictions
mean((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2) # mean prediction error
#0.01542
sd((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2)/sqrt(n.valid.y) # std error
# 0.0008
...
rmse
# 0.1242
####################Generalized Linear Regression (GLM), RMSE = 0.1163
pred.valid.ls1 <- predict(fit.glm.y.box, newdata = dataset_validation.y.box) # validation predictions
mean((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2) # mean prediction error
#0.01542
sd((dataset_validation.y.box$donation_amount_prediction_int - pred.valid.ls1)^2)/sqrt(n.valid.y) # std error
# 0.0008
...
#find out variable importance
summary(fit.cubist.y.box)
# find out model detials
fit.cubist.y.box
plot(varImp(fit.cubist.y.box,scale=F))
